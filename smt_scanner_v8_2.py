"""
SMT Setup Scanner v8.2
Changes from v8.1:
  1. SMT_IN_FVG entry: limit order at 50% of post-SMT FVG (not market entry)
  2. Swing point staleness: sw1 must be within STALENESS_LIMIT_MINS of sw2 on 1m SMT only
  3. FVG selection: prefer earliest valid FVG closest to SMT bar (not first found in lookahead)
  4. Duplicate signal fix: dedup now checks both direction AND instrument per bucket
"""
...
# ── MACRO BIAS (unchanged from v8.1) ─────────────────────────────────────────
def get_session_open_bias(df_full, signal_et, instrument):
    col_h = f'high_{instrument.lower()}'
    col_l = f'low_{instrument.lower()}'
    col_c = f'close_{instrument.lower()}'
    sig_date = signal_et.date()
    sig_t    = signal_et.hour*60+signal_et.minute
    ...
    if 480<=sig_t<630:
        ...
        open_bars = df_full[(df_full['et'].dt.date==sig_date) &
                            (df_full['et'].dt.hour==8) &
                            (df_full['et'].dt.minute<=5)]
        if open_bars.empty: return 'unclear'
        open_price = open_bars.iloc[0][col_c]

    elif 780<=sig_t<900:
        morning = df_full[(df_full['et'].dt.date==sig_date) &
                          (df_full['et'].dt.hour*60+df_full['et'].dt.minute>=480) &
                          (df_full['et'].dt.hour*60+df_full['et'].dt.minute<630)]
        if morning.empty: return 'unclear'
        ref_mid   = (morning[col_h].max()+morning[col_l].min())/2
        open_bars = df_full[(df_full['et'].dt.date==sig_date) &
                            (df_full['et'].dt.hour==13) &
                            (df_full['et'].dt.minute<=5)]
        if open_bars.empty: return 'unclear'
        open_price = open_bars.iloc[0][col_c]
    else:
...
if __name__ == '__main__':
    ...
    if not results.empty:
        ...
        out_path = '/mnt/user-data/outputs/smt_signals_v8_2.csv'
