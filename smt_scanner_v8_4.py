"""
SMT Setup Scanner v8.4
Changes from v8.3:
  - Added required 5m SMT confluence layer (STRICT mode).
  - Each 1m SMT signal must be confirmed by a 5m SMT in the same direction
    within the lookback window before the signal time.
  - 5m SMT uses the same wick-to-wick logic as 1m, with 1-bar timestamp leeway
    and 36-bar (3 hour) staleness window.
  - Signals blocked for: no 5m SMT, opposing 5m SMT, or conflicted 5m SMT.
  - Output CSV adds 5m SMT context: agreeing_5m_smt_time, agreeing_5m_smt_prices.
"""
...
STALENESS_LIMIT_MINS    = 60     # [v8.2] sw1 must be within 60 mins of sw2 on 1m
...
SMT_5M_LOOKBACK_BARS    = 36     # 3 hours of 5m bars
SMT_5M_TIME_GAP_BARS    = 1      # 1-bar leeway between sw1/sw2 on 5m
SMT_5M_MODE             = 'strict'  # 'strict' = require confirming 5m SMT
...
def check_5m_smt_confluence(es5, nq5, signal_et, trade_direction):
    """
    Looks back over the last SMT_5M_LOOKBACK_BARS of 5m bars BEFORE signal_et.
    Returns dict with status: 'agree' | 'oppose' | 'conflicted' | 'none'.
    """
    ...
    if len(es_win) < 5 or len(nq_win) < 5:
        return {'status':'none','smt_time':None,'smt_prices':None,
                'detail':'not enough 5m bars'}
    ...
    # Bullish 5m SMT: one instrument sweeps swing low, other holds
    if len(es_sl) >= 2 and len(nq_sl) >= 2:
        for (es_t1, es_p1, _) in reversed(es_sl[-3:]):
            for (es_t2, es_p2, _) in reversed(es_sl[-3:]):
                if es_t2 <= es_t1: continue
                for (nq_t1, nq_p1, _) in reversed(nq_sl[-3:]):
                    if _bars_apart_5m(es_t1, nq_t1) > SMT_5M_TIME_GAP_BARS: continue
                    for (nq_t2, nq_p2, _) in reversed(nq_sl[-3:]):
                        if nq_t2 <= nq_t1: continue
                        if _bars_apart_5m(es_t2, nq_t2) > SMT_5M_TIME_GAP_BARS: continue
                        if es_p2 < es_p1 and nq_p2 >= nq_p1:
                            bullish_smt = (es_t2,
                                f'ES sw1:{es_p1}@{str(es_t1)[11:16]} sw2:{es_p2}@{str(es_t2)[11:16]} | '
                                f'NQ sw1:{nq_p1}@{str(nq_t1)[11:16]} sw2:{nq_p2}@{str(nq_t2)[11:16]}')
                            break
                        if nq_p2 < nq_p1 and es_p2 >= es_p1:
                            bullish_smt = (nq_t2,
                                f'NQ sw1:{nq_p1}@{str(nq_t1)[11:16]} sw2:{nq_p2}@{str(nq_t2)[11:16]} | '
                                f'ES sw1:{es_p1}@{str(es_t1)[11:16]} sw2:{es_p2}@{str(es_t2)[11:16]}')
                            break
                    if bullish_smt: break
                if bullish_smt: break
            if bullish_smt: break

    # Bearish 5m SMT: one instrument sweeps swing high, other holds
    if len(es_sh) >= 2 and len(nq_sh) >= 2:
        for (es_t1, es_p1, _) in reversed(es_sh[-3:]):
            for (es_t2, es_p2, _) in reversed(es_sh[-3:]):
                if es_t2 <= es_t1: continue
                for (nq_t1, nq_p1, _) in reversed(nq_sh[-3:]):
                    if _bars_apart_5m(es_t1, nq_t1) > SMT_5M_TIME_GAP_BARS: continue
                    for (nq_t2, nq_p2, _) in reversed(nq_sh[-3:]):
                        if nq_t2 <= nq_t1: continue
                        if _bars_apart_5m(es_t2, nq_t2) > SMT_5M_TIME_GAP_BARS: continue
                        if es_p2 > es_p1 and nq_p2 <= nq_p1:
                            bearish_smt = (es_t2,
                        ...
                        if nq_p2 > nq_p1 and es_p2 <= es_p1:
                            bearish_smt = (nq_t2,
                                ...
                                f'ES sw1:{es_p1}@{str(es_t1)[11:16]} sw2:{es_p2}@{str(es_t2)[11:16]}')
                            break
                    if bearish_smt: break
                if bearish_smt: break
            if bearish_smt: break

    if bullish_smt and bearish_smt:
        return {'status':'conflicted','smt_time':None,'smt_prices':None,
                'detail':'both bullish and bearish 5m SMT in window'}

    want_bullish = (trade_direction == 'LONG')
    if want_bullish:
        if bullish_smt:
            return {'status':'agree','smt_time':bullish_smt[0],
                    'smt_prices':bullish_smt[1],'detail':'bullish 5m SMT confirms LONG'}
        if bearish_smt:
            return {'status':'oppose','smt_time':None,'smt_prices':None,
                    'detail':f'bearish 5m SMT opposes LONG'}
        return {'status':'none','smt_time':None,'smt_prices':None,'detail':'no 5m SMT'}
    else:
        if bearish_smt:
            return {'status':'agree','smt_time':bearish_smt[0],
                    'smt_prices':bearish_smt[1],'detail':'bearish 5m SMT confirms SHORT'}
        if bullish_smt:
            return {'status':'oppose','smt_time':None,'smt_prices':None,
                    'detail':f'bullish 5m SMT opposes SHORT'}
        return {'status':'none','smt_time':None,'smt_prices':None,'detail':'no 5m SMT'}
...
if __name__ == '__main__':
    print("SMT Scanner v8.4")
    print("="*60)
    print(f"Macro filter:     15m structural bias (3-TF framework)")
    print(f"5m SMT mode:      {SMT_5M_MODE}  (lookback={SMT_5M_LOOKBACK_BARS} bars, gap={SMT_5M_TIME_GAP_BARS} bar)")
    ...
    if not results.empty:
        ...
        out_path = '/mnt/user-data/outputs/smt_signals_v8_4.csv'
