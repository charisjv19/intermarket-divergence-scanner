"""
SMT Setup Scanner v8.6
Changes from v8.5:
  - Multiple sw2 candidates considered (last 5 confirmed swings, newest first)
    instead of just the most recent swing.
  - sw1 on the failing instrument now uses PARALLEL detection — found by
    matching timestamps with sw1 on the confirming instrument (within 2-bar
    tolerance), not by "most recent prior swing".
  - All broken sw1s on the confirming side are evaluated; signal fires if
    at least one has a "failed parallel" on the other instrument.
  - Primary sw1 = OLDEST failed-parallel (= most structurally significant break).
  - NEW: confirmations_count column = how many sw1s the divergence applies to.
  - NEW: alt_sw1_times = additional sw1 timestamps when multiple confirm.
"""
...
def run(es_path, nq_path):
    ...
    for i, row in sdf.iterrows():
        ...
        if smt:
            ...
            # Try NQ as confirming side (NQ broke prior low, ES failed)
...
if __name__ == '__main__':
    ...
    if not results.empty:
        ...
        out_path = '/mnt/user-data/outputs/smt_signals_v8_6.csv'
