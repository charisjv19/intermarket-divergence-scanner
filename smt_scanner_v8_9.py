"""
SMT Setup Scanner v8.9
Base: tagged v8.8 (3a209081). One change: SMT_IN_FVG membership.

v8.9: Pre-FVG membership is whether the confirming instrument's sw1 and/or
sw2 wick prices sit inside a pre-existing FVG [low, high]. One or both
qualifying is enough. v8.8 checked the confirmation-bar close instead.
Everything else is identical to v8.8: market at confirmation close
(sw2_conf_idx + 1), FVG-boundary SL (1.0 ES / 3.0 NQ buffer, 20/40-tick
floor), TP 1.5R, 15m/5m at the confirmation clock, no wait for
displacement. Close-in-FVG / neither-swing candidates are excluded and
do not fall through to FVG_AFTER_SMT.

Inherited v8.8 audit fixes:
  ...
  - [S2] Entry / stop / target protocol. SMT_IN_FVG now takes a MARKET entry at
    ...
    post-SMT FVG. Pre-FVG membership (v8.9) is the confirming-instrument
    sw1/sw2 wick prices inside a pre-FVG, looked up from the confirmation
    bar (sw2 + 1), not the outer-loop bar. smt_time is that same
...
# the pre-existing FVG that sw1 and/or sw2 wick sits in, then floored.
...
def compute_smt_in_fvg_stop(entry_price, pre_fvg_low, pre_fvg_high, direction,
    ...
    Formula is unchanged from v8.8. v8.9 membership is swing-wick-in-FVG,
    so entry (confirmation close) may sit outside the FVG; the min-stop
    floor still applies when that makes risk shorter than 20/40 ticks.
    """
    if direction == 'LONG':
...
def _price_in_fvg(lo, hi, px):
    return lo <= float(px) <= hi


# ── [v8.9] FIND PRE-EXISTING FVG (SMT_IN_FVG membership) ─────────────────────
def find_preexisting_fvg(sdf, smt_idx, direction, instrument, lookback, prices):
    """
    Looks back from smt_idx for the most recent pre-existing FVG whose
    [low, high] contains any of `prices`.

    v8.9 SMT_IN_FVG membership: pass the confirming-instrument sw1 and/or
    sw2 wick prices. One or both inside is enough. Same lookback, contig,
    pre-session, imbalance, and size guards as v8.8; only the inside-FVG
    check changed (v8.8 passed the confirmation-bar close).

    Callers pass the sw2 confirmation bar (sw2_conf_idx + 1) as smt_idx.
    ...
    for j in range(smt_idx-1, start, -1):
        ...
        imbal = (direction == 'LONG' and ph < nl) or (direction == 'SHORT' and pl > nh)
        if not imbal:
            continue
        lo, hi = (ph, nl) if direction == 'LONG' else (nh, pl)
        size = round(hi - lo, 2)
        if not (fvg_min <= size <= fvg_max):
            continue
        if any(_price_in_fvg(lo, hi, px) for px in prices):
            return dict(
                pre_fvg_found=True,
...
def run(es_path, nq_path):
    ...
    for i, row in sdf.iterrows():
        ...
        for c in candidates:
            ...
            pre_fvg = None
            close_only_no_swings = False
            if conf_ok:
                entry_price = round(float(conf_row[f'close_{instr_l}']), 2)
                if instr == 'ES':
                    sw1, sw2 = c['es_sw1_price'], c['es_sw2_price']
                else:
                    sw1, sw2 = c['nq_sw1_price'], c['nq_sw2_price']
                pre_fvg = find_preexisting_fvg(
                    sdf, conf_bar_idx, direction, instr, PRE_FVG_LOOKBACK, (sw1, sw2)
                )
                if pre_fvg is None:
                    # Close sits in a pre-FVG but neither swing does: v8.8
                    # would have fired SMT_IN_FVG. v8.9 excludes it entirely
                    # (no FVG_AFTER_SMT fall-through).
                    close_only_no_swings = find_preexisting_fvg(
                        sdf, conf_bar_idx, direction, instr, PRE_FVG_LOOKBACK,
                        (entry_price,),
                    ) is not None
            elif conf_row is not None:
                stale_filtered += 1  # SMT_IN_FVG only: next bar is not +1 minute

            if close_only_no_swings:
                continue

            if pre_fvg:
                # ── [v8.8 S2 / v8.9 membership] SMT_IN_FVG ────────────────────
                # MARKET entry at the confirmation-bar close. SL just outside
                # the pre-existing FVG that sw1/sw2 wick sits in, floored at
                # the shared 20/40-tick minimum. A post-SMT FVG is NOT required.
...
if __name__ == '__main__':
    print("SMT Scanner v8.9")
    print("="*60)
    print("SMT_IN_FVG membership: confirming-instrument sw1/sw2 wick in pre-FVG")
    ...
    if not results.empty:
        ...
        out_path = '/mnt/user-data/outputs/smt_signals_v8_9.csv'
