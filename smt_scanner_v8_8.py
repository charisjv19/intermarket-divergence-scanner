"""
SMT Setup Scanner v8.8
Seeded from v8.7. Scope per the Sep 2026 independent audit
(SMT_FVG_Strategy_Audit_Sep2026): structural/logic fixes S1, S2, S3, S11 only.
No threshold changes in this version (ES_FVG_MIN, NQ_FVG_MIN, etc. are frozen
until the May-June out-of-sample test).

Changes from v8.7:
  S3 (High): sw1 parallel timestamp tolerance tightened to an exact match.
     - SW1_PARALLEL_TOL_MINS changed from 2 to 0, so the failing-instrument
       sw1 must occur on the SAME bar as the confirming-instrument sw1 (zero
       tolerance), matching the sw2 alignment (MAX_SW_TIME_GAP_MINS = 0) and
       the project's structural-correctness invariant. Removes signals whose
       paired sw1s were 1-2 minutes apart.

Pending (not yet implemented in this file): S1 (session-aware sw1 walk-back),
S2 (SMT_IN_FVG market-entry protocol), S11 (emit smt5m_prices).
...
if __name__ == '__main__':
    ...
    print(f"Parallel sw1 tol: {SW1_PARALLEL_TOL_MINS} mins  [v8.8 / S3: exact match]")
